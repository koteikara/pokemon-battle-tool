...
type WorldCupPredictor = {
  id: string;
  name: string;
};

type WorldCupPredictionState = Record<string, Record<string, WorldCupPredictionPick>>;

type WorldCupSavedState = {
  predictors: WorldCupPredictor[];
  predictions: WorldCupPredictionState;
};

const WORLD_CUP_STORAGE_KEY = "pokemon-battle-tool-world-cup-v1";
const MAX_WORLD_CUP_PREDICTORS = 3;

function todayInputValue() {
  return new Date().toISOString().slice(0, 10);
}

function createWorldCupPredictor(name = ""): WorldCupPredictor {
  return { id: crypto.randomUUID(), name };
}

function readWorldCupSavedState(): WorldCupSavedState {
  if (typeof window === "undefined") {
    return { predictors: [createWorldCupPredictor("予想する人1")], predictions: {} };
  }
  ...
  try {
    const parsed = JSON.parse(raw) as Partial<WorldCupSavedState>;
    const predictors = Array.isArray(parsed.predictors)
      ? parsed.predictors
          .filter((predictor): predictor is WorldCupPredictor =>
            typeof predictor?.id === "string" && typeof predictor?.name === "string",
          )
          .slice(0, MAX_WORLD_CUP_PREDICTORS)
      : [];

    return {
      predictors: predictors.length > 0 ? predictors : [createWorldCupPredictor("予想する人1")],
...
function formatWorldCupKickoff(value: string) {
...
function WorldCupScreen() {
  ...
  const [predictors, setPredictors] = useState<WorldCupPredictor[]>(() => readWorldCupSavedState().predictors);
  const [predictions, setPredictions] = useState<WorldCupPredictionState>(() => readWorldCupSavedState().predictions);
  ...
  useEffect(() => {
    window.localStorage.setItem(
      ...
      JSON.stringify({ predictors, predictions }),
    );
  ...
  useEffect(() => {
    ...
    fetchWorldCupMatches(selectedDate)
      .then((nextMatches) => {
        ...
        setLastUpdated(new Intl.DateTimeFormat("ja-JP", {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
        }).format(new Date()));
      ...
      .catch(() => {
        if (!active) return;
        setMatches([]);
        setError("試合結果を取得できませんでした。あとでもう一度ためしてください。");
      })
      .finally(() => {
    ...
    return predictors.map((predictor) => {
      const checked = matches
        .map((match) => {
          ...
          if (actual === null || pick === "unset") return null;
          return actual === pick;
        })
      ...
      return {
        predictor,
        correct: checked.filter(Boolean).length,
  ...
  function updatePredictorName(id: string, name: string) {
  ...
  function addPredictor() {
    setPredictors((current) => {
      if (current.length >= MAX_WORLD_CUP_PREDICTORS) return current;
      return [...current, createWorldCupPredictor(`予想する人${current.length + 1}`)];
    });
  }
  ...
  function removePredictor(id: string) {
    ...
    setPredictions((current) => {
      const next: WorldCupPredictionState = {};
  ...
  function updatePrediction(matchId: string, predictorId: string, pick: WorldCupPredictionPick) {
    setPredictions((current) => ({
      ...current,
      [matchId]: {
        ...(current[matchId] ?? {}),
  ...
  return (
    <section className="card worldcup-screen">
      <div className="worldcup-hero">
        <div>
          <div className="worldcup-kicker">WORLD CUP DAILY</div>
          <h2 className="card-title">ワールドカップ勝敗予想</h2>
          <p className="subtext">
            毎日の試合を3人まで予想できます。試合後は自動取得した結果と照合します。
          </p>
        </div>
        <div className="worldcup-ball" aria-hidden="true">⚽</div>
      </div>

      <div className="worldcup-controls">
        <label className="field-label" htmlFor="worldcup-date">日にち</label>
        <input
          id="worldcup-date"
          className="field-input"
          type="date"
        ...
        />
        <button type="button" className="btn-secondary" onClick={refreshMatches} disabled={loading}>
          結果を更新
        </button>
      </div>

      <div className="worldcup-source-note">
        結果データ：ESPNの公開スコアボードAPIを参照。公式同等ではなく、表示できない日は手動で日付を変えてください。
        {lastUpdated ? <span> 最終更新 {lastUpdated}</span> : null}
      </div>

      <div className="worldcup-predictors">
        <div className="section-title-row">
          <h3>予想する人</h3>
          <button type="button" className="btn-secondary" onClick={addPredictor} disabled={predictors.length >= MAX_WORLD_CUP_PREDICTORS}>
            人を追加
          </button>
        </div>
        <div className="worldcup-predictor-grid">
          {predictors.map((predictor, index) => (
            <div className="worldcup-predictor-card" key={predictor.id}>
              <label className="field-label" htmlFor={`predictor-${predictor.id}`}>人 {index + 1}</label>
              <input
                id={`predictor-${predictor.id}`}
                className="field-input"
                value={predictor.name}
                onChange={(event) => updatePredictorName(predictor.id, event.target.value)}
                maxLength={16}
              />
              <div className="worldcup-score-chip">
                {scoreBoard[index]?.correct ?? 0}/{scoreBoard[index]?.total ?? 0} 的中
              </div>
              {predictors.length > 1 ? (
                <button type="button" className="btn-danger" onClick={() => removePredictor(predictor.id)}>
                  けす
                </button>
              ) : null}
            </div>
      ...
      {loading ? <div className="empty-notice">試合を読みこみ中...</div> : null}
      {error ? <div className="empty-notice empty-notice--danger">{error}</div> : null}
      {!loading && !error && matches.length === 0 ? (
        <div className="empty-notice">この日の試合は見つかりませんでした。</div>
      ) : null}
      ...
      <div className="worldcup-match-list">
        {matches.map((match) => {
          ...
          return (
            <article className="worldcup-match-card" key={match.id}>
              <div className="worldcup-match-head">
                <div>
                  <div className="worldcup-match-time">{formatWorldCupKickoff(match.date)}</div>
                  <h3>{match.homeTeam} vs {match.awayTeam}</h3>
                  <p>{match.venue}</p>
                </div>
                <div className={`worldcup-status worldcup-status--${match.status}`}>
                  {match.statusText}
                </div>
              </div>

              <div className="worldcup-result-line">
                <span>結果</span>
                <strong>
              ...
              <div className="worldcup-prediction-table">
                {predictors.map((predictor) => {
                  const pick = predictions[match.id]?.[predictor.id] ?? "unset";
                  const checked = actualWinner !== null && pick !== "unset";
                  const hit = checked && actualWinner === pick;
                  return (
                    <div className="worldcup-prediction-row" key={`${match.id}-${predictor.id}`}>
                      <div className="worldcup-predictor-name">{predictor.name || "なまえなし"}</div>
                      <select
                        className="field-input"
                        value={pick}
                        onChange={(event) => updatePrediction(match.id, predictor.id, event.target.value as WorldCupPredictionPick)}
                        aria-label={`${predictor.name}の予想`}
                      >
                      ...
                      <div className={`worldcup-check ${checked ? (hit ? "worldcup-check--hit" : "worldcup-check--miss") : ""}`}>
                        {checked ? (hit ? "的中" : "はずれ") : getPickLabel(pick, match)}
                      </div>
                    </div>
                  );
                })}
